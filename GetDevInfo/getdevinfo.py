...
class Main():
    ...
    def GetDescription(self, Disk, DiskLineNumber=None, DiskList=None):
        ...
        for LineNumber in self.DescriptionLinesList:
            ...
            if DiskLineNumber - LineNumber < 10:
                ...
                logger.warning("GetDevInfo: Main().GetDescription(): Found probable wrong description: "+' '.join(self.Output[LineNumber].split()[1:])+". Ignoring it and returning 'Unknown'...")
    ...
    def GetPartitionScheme(self, Disk, DiskLineNumber=None): #*** Maybe get rid of this and just use parted for this in StartupScripts ***
        ...
        #Look for the information using the Disk's line number.
        for Number in self.CapabilitiesLinesList:
            #Ignore the line number if it is before the Disk name...
            if Number < DiskLineNumber:
                if self.CapabilitiesLinesList[-1] != Number:
                    continue
            ...
            if Number - DiskLineNumber < 10:
                PartitionScheme = self.Output[Number].split(":")[-1]
