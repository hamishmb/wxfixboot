...
def FindMissingFSCKModules():
    ...
    logger.info("HelperBackendTools: FindMissingFSCKModules(): Looking for missing FSCK modules to ignore...")
    FailedList = []
    ...
    for Disk in Keys:
        ...
        if DiskInfo[Disk]["FileSystem"] not in ("Unknown", "N/A"):
            ...
            if CoreTools.StartProcess("which fsck."+DiskInfo[Disk]["FileSystem"], ShowOutput=False) != 0:
                ...
                logger.warning("HelperBackendTools: FSCKModules(): Couldn't find FSCK module fsck."+DiskInfo[Disk]["FileSystem"]+"! Adding it to the list of missing modules...")
                FailedList.append("fsck."+DiskInfo[Disk]["FileSystem"])

            else:
                logger.debug("HelperBackendTools: FSCKModules(): Found fsck."+DiskInfo[Disk]["FileSystem"]+"...")

    #Return the list, so FSCheck functions know which FSes to ignore.
    logger.info("HelperBackendTools: FindMissingFSCKModules(): Done! Missing FSCK modules: "+', '.join(FailedList))
    return FailedList

def FindCheckableFileSystems():
    """Find all checkable filesystems, and then return them to EssentialBackendTools().FileSystemCheck()"""
    logger.info("HelperBackendTools: FindCheckableFileSystems(): Finding and returning all filesystems/partitions that can be checked...")
        ...
        if "fsck."+DiskInfo[Disk]["FileSystem"] in MissingFSCKModules:
            ...
            if CoreTools.IsMounted(Disk) == False:
                ...
                if CoreTools.Unmount(Disk) != 0:
                    logger.warning("HelperBackendTools: FindCheckableFileSystems(): Failed to unmount "+Disk+", which is necessary for safe disk checking! Ignoring it...")
                    CheckTheFS = False
                    RemountPartitionAfter = False
    ...
    logger.info("HelperBackendTools: FindCheckableFileSystems(): Done! Filesystems that won't be checked: "+'\n'.join(DoNotCheckList)+"...")
    return FileSystemsToCheck

def HandleFilesystemCheckReturnValues(ExecCmds, Retval, Partition):
    ...
    if Retval in (1, 2, 3) and ExecList[0] != "badblocks":
        if ExecList[0] == "xfs_repair":
            ...
            logger.warning("HelperBackendTools: HandleFilesystemCheckReturnValues(): xfs_repair detected filesystem corruption on FileSystem: "+Partition+". It's probably (and hopefully) been fixed, but we're logging this anyway.")
            ...
            #Fixed Errors.
            logger.info("HelperBackendTools: HandleFilesystemCheckReturnValues(): "+ExecList[0]+" successfully fixed errors on the partition: "+Partition+". Continuing...")
        ...
        logger.error("HelperBackendTools: HandleFilesystemCheckReturnValues(): "+ExecList[0]+" Errored with exit value "+unicode(Retval)+"! This could indicate filesystem corruption or bad sectors!")

        if BootloaderOperations:
            logger.error("HelperBackendTools: HandleFilesystemCheckReturnValues(): Asking the user whether to skip bootloader operations...")
            ...
            if Result:
                ...
                logger.warning("HelperBackendTools: HandleFilesystemCheckReturnValues(): User disabled bootloader operations as recommended, due to bad sectors/HDD problems/FS Checker problems...")
                ...
                logger.warning("HelperBackendTools: HandleFilesystemCheckReturnValues(): User ignored the warning and went ahead with bootloader modifications (if any) anyway, even with possible HDD problems/Bad sectors! This is a REALLY bad idea, but we'll do it anyway, as requested...")
...
def WriteFSTABEntryForUEFIPartition(OS, MountPoint):
    ...
    logger.info("HelperBackendTools: WriteFSTABEntryForUEFIPartition(): Preparing to write an fstab entry for the UEFI partition ("+OSInfo[OS]["EFIPartition"]+")...")
    ...
    if WriteEntry == False:
        #We don't!
        logger.info("HelperBackendTools: WriteFSTABEntryForUEFIPartition(): fstab entry already present! Skipping...")
        fstab.close()

    else:
        #We do. If we can use the UUID, then we will, but otherwise we'll use the standard device name.
        logger.info("HelperBackendTools: WriteFSTABEntryForUEFIPartition(): Writing fstab entry...")
        ...
        if DiskInfo[OSInfo[OS]["EFIPartition"]]["UUID"] != "Unknown":
            logger.info("HelperBackendTools: WriteFSTABEntryForUEFIPartition(): Using UUID to prevent problems down the line...")
            NewFileContents.append("UUID="+DiskInfo[OSInfo[OS]["EFIPartition"]]["UUID"]+" /boot/efi vfat defaults 0 2\n")

        else:
            logger.warning("HelperBackendTools: WriteFSTABEntryForUEFIPartition(): We have no UUID for the UEFI Partition: "+OSInfo[OS]["EFIPartition"]+"! This isn't good, and may cause problems down the line. Continuing anyway, using device name instead...")
        ...
        logger.info("HelperBackendTools: WriteFSTABEntryForUEFIPartition(): Done!")
...
def BackupUEFIFiles(MountPoint):
    ...
    logger.info("HelperBackendTools: BackupUEFIFiles(): Backing up UEFI Files...")
    ...
    logger.info("HelperBackendTools: BackupUEFIFiles(): Backing up "+MountPoint+"/boot/efi/boot/boot*.efi...")
    ...
    if os.path.isfile(MountPoint+"/boot/efi/EFI/boot/boot*.efi"):
        if CoreTools.StartProcess("cp -v "+MountPoint+"/boot/efi/EFI/boot/boot*.efi "+MountPoint+"/boot/efi/EFI/boot/bkpbootx64.efi", ShowOutput=False) != 0:
            ...
            logger.error("HelperBackendTools: BackupUEFIFiles(): Failed to backup failsafe UEFI boot file! Warning user and continuing...")
    ...
    logger.info("HelperBackendTools: BackupUEFIFiles(): Backing up Windows's boot files if they exist...")
    ...
    if os.path.isfile(MountPoint+"/boot/efi/EFI/Microsoft/boot/bootmgfw.efi"):
        if CoreTools.StartProcess("cp -v "+MountPoint+"/boot/efi/EFI/Microsoft/boot/bootmgfw.efi "+MountPoint+"/boot/efi/EFI/Microsoft/boot/bkpbootmgfw.efi", ShowOutput=False) != 0:
            #Log and warn user if this went wrong.
            logger.error("HelperBackendTools: BackupUEFIFiles(): Failed to backup Windows's UEFI boot files! Warning user and continuing...")
    ...
    logger.info("HelperBackendTools: BackupUEFIFiles(): Done!")

def ManageUEFIFiles(OS, MountPoint):
    """Manage UEFI bootloader files."""
    logger.info("HelperBackendTools: ManageUEFIFiles(): Copying UEFI Files to UEFIBootDir...")
    ...
    if BootloaderInfo[OS]["Settings"]["NewBootloader"] == "ELILO":
        ...
        logger.info("HelperBackendTools: ManageUEFIFiles(): Copying elilo.efi, elilo.conf and elilomenu.msg to "+UEFIBootDir+"...")

        if CoreTools.StartProcess("cp -v "+SourceDir+"/elilo.efi "+UEFIBootDir+"/bootx64.efi", ShowOutput=False) != 0:
            logger.error("HelperBackendTools: ManageUEFIFiles(): Failed to copy "+SourceDir+"/elilo.efi to "+UEFIBootDir+"/bootx64.efi! Attempting to continue anyway...")
        ...
        if CoreTools.StartProcess("cp -v "+SourceDir+"/elilo.conf "+UEFIBootDir+"/", ShowOutput=False) != 0:
            logger.error("HelperBackendTools: ManageUEFIFiles(): Failed to copy "+SourceDir+"/elilo.conf to "+UEFIBootDir+"/! Attempting to continue anyway...")
            DialogTools.ShowMsgDlg(Kind="error", Message="WxFixBoot failed to copy one of the new bootloader's UEFI files to the failsafe directory! This could potentially be a problem, but it's probably fine. Click okay to continue.")

        if CoreTools.StartProcess("cp -v /usr/share/wxfixboot/sampleconfig/elilomenu.msg "+UEFIBootDir+"/", ShowOutput=False) != 0:
            logger.error("HelperBackendTools: ManageUEFIFiles(): Failed to copy elilomenu.msg to "+UEFIBootDir+"! Attempting to continue anyway...")
            DialogTools.ShowMsgDlg(Kind="error", Message="WxFixBoot failed to copy one of the new bootloader's UEFI files to the failsafe directory! This could potentially be a problem, but it's probably fine. Click okay to continue.")

        if CoreTools.StartProcess("cp -v /usr/share/wxfixboot/sampleconfig/elilomenu.msg "+SourceDir+"/", ShowOutput=False) != 0:
            logger.error("HelperBackendTools: ManageUEFIFiles(): Failed to copy elilomenu.msg to "+SourceDir+"/! Attempting to continue anyway...")
            DialogTools.ShowMsgDlg(Kind="error", Message="WxFixBoot failed to copy one of the new bootloader's UEFI files to the failsafe directory! This could potentially be a problem, but it's probably fine. Click okay to continue.")
        ...
        if BootloaderInfo[OS]["Bootloader"] == "GRUB-UEFI":
            if CoreTools.StartProcess("rm -v "+SourceDir+"/grub*.efi", ShowOutput=False) != 0:
                logger.warning("HelperBackendTools: ManageUEFIFiles(): Failed to remove "+SourceDir+"/grub*.efi! Attempting to continue anyway...")

    elif BootloaderInfo[OS]["Settings"]["NewBootloader"] == "GRUB-UEFI":
        #We need to copy grub*.efi to UEFIBootDir.
        logger.info("HelperBackendTools: ManageUEFIFiles(): Copying grub*.efi to "+UEFIBootDir+"...")

        if CoreTools.StartProcess("cp -v "+SourceDir+"/grub*.efi "+UEFIBootDir+"/bootx64.efi", ShowOutput=False) != 0:
            logger.error("HelperBackendTools: ManageUEFIFiles(): Failed to copy "+SourceDir+"/grub*.efi to "+UEFIBootDir+"/bootx64.efi! Attempting to continue anyway...")
        ...
        if BootloaderInfo[OS]["Bootloader"] == "ELILO":
            if CoreTools.StartProcess("rm -v "+SourceDir+"/elilo*", ShowOutput=False) != 0:
                logger.warning("HelperBackendTools: ManageUEFIFiles(): Failed to remove "+SourceDir+"/elilo*! Attempting to continue anyway...")

    logger.info("HelperBackendTools: ManageUEFIFiles(): Done!")
