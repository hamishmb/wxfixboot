...
class MainBackendThread(threading.Thread):
    ...
    ####################Essential Functions, that must be run before doing bootloader oparations.#################### *** Move to seperate file ***
    ...
    def BadSectorCheck(self): #*** This is very duplicated with QuickFSCheck, can we merge them and put a check in? ***
        ...
        wx.CallAfter(self.ParentWindow.UpdateCurrentOpText, Message="Preparing for Bad Sector Check...")
        ...
        for Element in CheckList:
            ...
            wx.CallAfter(self.ParentWindow.UpdateCurrentOpText, Message="Checking Partition: "+Partition)
            ...
            if ExecList != ['None']:
                retval = CoreBackendTools().StartThreadProcess(ExecList)

                #Check the return values, and run the handler if needed.
                if retval == 0:
                    #Success.
                    logger.info("MainBackendThread().BadSectorCheck(): Checked partition: "+Partition+" No Errors Found!")

                else:
                    global OSsForBootloaderRemoval
                    global OSsForBootloaderInstallation

                    OSsForBootloaderRemoval, OSsForBootloaderInstallation = self.HandleFilesystemCheckReturnValues(ExecList=ExecList, Retval=retval, Partition=Partition, OSsForBootloaderRemoval=OSsForBootloaderRemoval, OSsForBootloaderInstallation=OSsForBootloaderInstallation)
        ...
        #Update Current Operation Text.
        wx.CallAfter(self.ParentWindow.UpdateCurrentOpText, Message="Finished Bad Sector Check!")
    ...
    def PrepareForBootloaderInstallation(self):
        ...
        if DisableBootloaderOperations:
            ...
            if OSListWithPackageManagers == []:
                ...
                global OSsForBootloaderInstallation
                OSsForBootloaderInstallation = self.AskUserForBootloaderInstallationOSs(OSListWithPackageManagers, UpdateBootloader, ReinstallBootloader, OSsForBootloaderRemoval)
                wx.CallAfter(self.ParentWindow.UpdateCurrentProgress, 100)
    ...
    def GetOldBootloaderConfig(self):
        ...
        wx.CallAfter(self.ParentWindow.UpdateCurrentOpText, Message="Preparing to get bootloader config...")
        ...
        wx.CallAfter(self.ParentWindow.UpdateCurrentOpText, Message="Getting old bootloader config...")
        ...
        for OS in OSsForBootloaderRemoval:
            ...
            if LiveDisk == False and Partition == AutoRootFS:
                ...
                if Retval != 0:
                    #Ignore this partition.
                    logger.warning("MainBackendThread().GetOldBootloaderConfig(): Failed to mount "+Partiton+"! Ignoring this partition...")
                    continue
        ...
        wx.CallAfter(self.ParentWindow.UpdateCurrentOpText, Message="Determining configuration to use...")
        ...
        if BootloaderTimeout == -1:
            if len(TimeoutsList) == 0:
                ...
                logger.info("MainBackendThread().GetOldBootloaderConfig(): Using user's bootloader timeout value: "+unicode(BootloaderTimeout))

            elif len(TimeoutsList) == 1:
                #As there is one, do what the user said, and set it directly.
                BootloaderTimeout = int(TimeoutsList[0])
                logger.info("MainBackendThread().GetOldBootloaderConfig(): Using only bootloader timeout value found: "+unicode(BootloaderTimeout))
                ...
                if Result == "WxFixBoot's Default (10)":
                    ...
                    logger.info("MainBackendThread().GetOldBootloaderConfig(): Using user chosen bootloader timeout value: "+unicode(BootloaderTimeout))
